...
uint32_t fifo_write(_fifo_t *pfifo, const uint8_t *pbuf, uint32_t size)
{
	...
	while(w_size-- > 0)
	{
		*pfifo->pwrite++ = *pbuf++;//向缓冲区写入数据
		if (pfifo->pwrite >= (pfifo->buf + pfifo->buf_size)) //写入数据大于可缓冲量，溢出
		{
			pfifo->pwrite = pfifo->buf;//重新返回到开始处
		}
        pfifo->occupy_size++;//写入量加一
	}
    if (pfifo->unlock != NULL)//如果有解锁函数
        pfifo->unlock();//执行解除锁定函数
	return size;
}
